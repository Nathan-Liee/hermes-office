...
> = {
  ...
  "todo-board": {
    ...
    featured: true,
    editorBadge: "HO3D test",
    hideStats: true,
  },
  "task-manager": {
    category: "Productivity",
    tagline:
      "Turns actionable requests into persistent shared tasks that power the HO3D Kanban board.",
    capabilities: [
...
export const buildSkillMarketplaceEntry = (
  ...
  if (packagedSkill && !skill.baseDir.trim()) {
    missingDetails.unshift(
      "Install this packaged HO3D skill to make it available on the gateway.",
    );
  }
  return {
    skill,
...
export const buildSkillMarketplaceCollections = (
  ...
  const ho3d = entries.filter((entry) =>
    getPackagedSkillBySkillKey(entry.skill.skillKey),
  );
  if (ho3d.length > 0) {
    collections.push({ id: "ho3d", label: "HO3D", entries: ho3d });
  }
