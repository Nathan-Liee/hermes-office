...
const STORE_DIR = path.join("ho3d", "task-manager");
...
const normalizeTaskRecord = (value: unknown): SharedTaskRecord | null => {
  ...
  return {
    ...
    source: isTaskBoardSource(record.source) ? record.source : "ho3d_manual",
...
export const upsertSharedTask = (
  ...
  const rawSource = (task.source as TaskBoardSource | undefined) ?? existing?.source ?? "ho3d_manual";
  ...
  const next: SharedTaskRecord = {
    ...
    source: isTaskBoardSource(rawSource) ? rawSource : "ho3d_manual",
